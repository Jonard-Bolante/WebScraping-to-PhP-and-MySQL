...
# WRITES TO mySQL
try:
    cnx = mysql.connector.connect(host='localhost', user='jonard', password='Bolante.9171996', database='JonardDBZ')
    cursor = cnx.cursor()
    query = 'INSERT INTO JonardDBZ.book(state,city,weather,temperature,humidity,pressure) VALUES (%s,%s,%s,%s,%s,%s)'
    cursor.execute(query, (data[0],data[1],data[2],data[3],data[4],data[5]))


#    query = 'UPDATE myData SET weather=%s temp=%s humid=%s press=%s WHERE state=%s city=%s'
